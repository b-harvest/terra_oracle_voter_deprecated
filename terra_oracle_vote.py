...
hardfix_active_set = ["uusd", "ukrw", "usdr",
                      "umnt"]  # hardfix the active set. does not care about stop_oracle_trigger_recent_diverge
chain_id = "soju-0012"
round_block_num = 5.0
...
def telegram(message):
    try:
        requestURL = "https://api.telegram.org/bot" + str(
            telegram_token) + "/sendMessage?chat_id=" + telegram_chat_id + "&text="
...
def get_my_current_prevotes():
    try:
        result = json.loads(
            requests.get(str(rpc_address) + "oracle/voters/" + str(validator) + "/prevotes").text)
        result_vote = []
...
def get_fx_rate():
    ...
    try:
        # get currency rate
        url = "https://fcsapi.com/api/forex/latest?symbol=USD/KRW,USD/EUR,USD/CNY,USD/JPY&access_key=" + str(
            fcsapi_key)
        api_result = json.loads(requests.get(url).text)
        result_real_fx = {"USDUSD": 1.0, "USDKRW": 1.0, "USDEUR": 1.0, "USDCNY": 1.0, "USDJPY": 1.0,
                          "USDSDR": 1.0, "USDMNT": 1.0}
...
def get_coinone_luna_price():
    ...
    try:
        ...
        luna_price = {"base_currency": "ukrw", "exchange": "coinone", "askprice": askprice,
                      "bidprice": bidprice, "midprice": midprice}
        luna_base = "USDKRW"
...
def get_gopax_luna_price():
    ...
    try:
        ...
        midprice = (askprice + bidprice) / 2.0
        luna_price = {"base_currency": "ukrw", "exchange": "gopax", "askprice": askprice,
                      "bidprice": bidprice, "midprice": midprice}
        luna_base = "USDKRW"
        luna_midprice_krw = float(luna_price["midprice"])
    except:
...
def get_gdac_luna_price():
    ...
    try:
        ...
        bidprice = float(luna_result["bid"][0]["price"])
        midprice = (askprice + bidprice) / 2.0
        luna_price = {"base_currency": "ukrw", "exchange": "gdac", "askprice": askprice, "bidprice": bidprice,
                      "midprice": midprice}
        luna_base = "USDKRW"
        luna_midprice_krw = float(luna_price["midprice"])
    except:
...
def broadcast_prevote(hash):
    msg_list = []
    for denom in active:
        msg_list.append({"type": "oracle/MsgExchangeRatePrevote",
                         "value": {"hash": str(hash[denom]), "denom": str(denom), "feeder": feeder,
                                   "validator": validator}})
    tx_json = {"type": "core/StdTx", "value": {"msg": msg_list,
                                               "fee": {"amount": [{"denom": fee_denom, "amount": fee_amount}],
                                                       "gas": fee_gas}, "signatures": [], "memo": ""}}
...
def broadcast_all(vote_price, vote_salt, prevote_hash):
    ...
    for denom in active:
        msg_list.append({"type": "oracle/MsgExchangeRateVote",
                         "value": {"exchange_rate": str(vote_price[denom]), "salt": str(vote_salt[denom]),
                                   "denom": denom, "feeder": feeder, "validator": validator}})
        hash_result[denom] = get_hash(str(vote_salt[denom]), str(vote_price[denom]), denom, validator)
    for denom in active:
        msg_list.append({"type": "oracle/MsgExchangeRatePrevote",
                         "value": {"hash": str(prevote_hash[denom]), "denom": str(denom), "feeder": feeder,
                                   "validator": validator}})
    tx_json = {"type": "core/StdTx", "value": {"msg": msg_list,
                                               "fee": {"amount": [{"denom": fee_denom, "amount": fee_amount}],
                                                       "gas": fee_gas}, "signatures": [], "memo": ""}}
...
while True:
    ...
    if next_height_round > last_prevoted_round and ((current_round + 1) * round_block_num - height == 0 or (
        ...
        ts = time.time()
        p = Pool(5)
        res_fx, res_sdr, res_coinone, res_gopax, res_gdac = p.map(get_data, ["get_fx_rate", "get_sdr_rate",
        ...
        if all_err_flag == False:
            ...
            try:
                ...
                result = {"index": int(ts / 60), "timestamp": ts, "block_height": latest_block_height,
                          "block_time": latest_block_time, "swap_price_compare": swap_price_compare,
                          "real_fx": real_fx, "luna_price_list": [coinone_luna_price, gopax_luna_price]}
            except:
                ...
                for prices in result["swap_price_compare"]:
                    if prices["market"] == denom:
                        ...
                        if abs(prices["market_price"] / prices[
                            ...
                            printandflush("prevoting " + denom + " : " + str(
        ...
        if currentmisses > misses:
            ...
            alarm_content = "Terra Oracle misses went from " + str(misses) + " to " + str(
                currentmisses) + "(" + str(misspercentage) + "%)"
            printandflush(alarm_content)
            if alertmisses == True:
                telegram(alarm_content)
                slack(alarm_content)
        ...
        printandflush(str(height) + " : wait " + str(
            (current_round + 1) * round_block_num - height) + " blocks until this round ends...")
